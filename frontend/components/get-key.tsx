...
export default function GetKey({ handleSubmit }: { handleSubmit: (event: FormEvent) => Promise<boolean | undefined> }) {
    ...
    useEffect(() => {
        if (user) {
            ...
            console.log('Auth0 state:', {
                ...
                error,
                user,
                hasEmailVerified: user?.email_verified !== undefined,
            ...
            if (user.email_verified === true) {
                ...
                // If email is not verified, suggest refreshing the session
                console.log('Email not verified, user may need to refresh session after verification');
            }
        }
    ...
    const handleRefreshSession = async () => {
        ...
        try {
            // Try to refresh the Auth0 session programmatically
            console.log('Attempting to refresh Auth0 session...');
            ...
            if (response.ok) {
                // If successful, reload the page to get the fresh session
                console.log('Session refresh successful, reloading page...');
                window.location.reload();
            } else {
                // If that doesn't work, try a full page reload
                console.log('Session refresh failed, forcing page reload...');
                window.location.reload();
            }
        } catch (error) {
