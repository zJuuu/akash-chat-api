    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (saved) {
          ...
          const ageInHours = (Date.now() - parsed.timestamp) / (60 * 60 * 1000);
          const maxAgeInHours = 7 * 24; // 7 days
          
          console.log(`[UserProvider] SessionStorage age: ${ageInHours.toFixed(2)} hours (max: ${maxAgeInHours} hours)`);
          
          // Only restore if it's recent (within 7 days to match session expiration)
          if (Date.now() - parsed.timestamp < 7 * 24 * 60 * 60 * 1000) {
            setPersistedAuthState(parsed.state);
            console.log('[UserProvider] Restored persisted auth state');
          } else {
            sessionStorage.removeItem('akash-auth-state');
            console.log('[UserProvider] Removed expired persisted auth state');
          }
        }
      } catch (error) {
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      if (auth0User) {
        ...
        const baseUrl = window.location.origin; // Use current origin (localhost:3000)
        console.log('[UserProvider] Current window.location.origin:', baseUrl);
        console.log('[UserProvider] Current window.location.href:', window.location.href);
        const logoutUrl = `/api/auth/logout?returnTo=${encodeURIComponent(baseUrl)}`;
        console.log('[UserProvider] Redirecting to Auth0 logout:', logoutUrl);
        console.log('[UserProvider] Full logout URL:', window.location.origin + logoutUrl);
  ...
  const ensureAuth0UserInLiteLLM = useCallback(async (user: any) => {
    if (!user?.sub) {
      console.warn('[UserProvider] Auth0 user has no sub (user ID), skipping LiteLLM creation');
      return;
    }

    try {
      console.log('[UserProvider] Checking if Auth0 user exists in LiteLLM:', {
        auth0UserId: user.sub.substring(0, 12) + '...',
      });
      ...
      if (response.ok) {
        const userData = await response.json();
        console.log('[UserProvider] Auth0 user already exists in LiteLLM', {
          userId: userData._id,
          authType: userData.authType,
          keyCount: userData.apiKeys?.length || 0
        });
        
        // If the user exists but shows as non-auth0, this might be a data inconsistency
        if (userData.authType !== 'auth0') {
          console.warn('[UserProvider] Auth0 user exists but has wrong authType:', userData.authType);
        }

      } else {
        console.error('[UserProvider] Unexpected response when checking Auth0 user:', response.status);
      }
    } catch (error) {
